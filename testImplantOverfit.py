...
def run(dataset: InMemoryDataset,
    ...
    for seed in tqdm(seeds):
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            loss = train(model, optimizer, dataset.data, key = "train")
            # trainD(model, optimizer, dataset.data)
            if not args.fixT:
                val_loss = train(model, optimizer_val, dataset.data, key = "val")
            eval_dict = evaluate(model, dataset.data, test)

            eval_dict["t"] = (np.mean(model.layers[0].diffusion.t.data.cpu().numpy()) + np.mean(model.layers[1].diffusion.t.data.cpu().numpy())) / 2
            eval_dict["train_loss"] = (loss.data.cpu().numpy()).item()
            eval_dict["val_loss"] = (val_loss.data.cpu().numpy()).item()
            if epoch % 1 == 0 and args.debugInfo:
...
results = run(
    dataset,
    model,
    seeds=test_seeds[0:1] if config['test'] else val_seeds,
