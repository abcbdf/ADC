...
from ModModels import GCN, GAT, JKNet, ARMA
...
def trainD(model: torch.nn.Module, optimizer: Optimizer, data: Data):
    model.train()
    optimizer.zero_grad()
    logits = model(data)
    loss = F.nll_loss(logits[data['train_mask']+data['val_mask']], data.y[data['train_mask']+data['val_mask']])
    # for layer in model.layers:
    #     loss = loss - layer.t
    # print("loss: " + str(loss))
    loss.backward()
